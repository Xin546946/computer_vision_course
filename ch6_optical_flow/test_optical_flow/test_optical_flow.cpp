...
int main(int argc, char **argv) {
  ...
  cv::goodFeaturesToTrack(img_gray_prev, feature_points_prev, 50, 0.01, 10,
                          cv::Mat());
  ...
  while (true) {
    ...
    cv::calcOpticalFlowPyrLK(img_gray_prev, img_gray_curr, feature_points_prev,
                             feature_points_curr, status, err, cv::Size(21, 21),
                             3);
    for (int i = 0; i < feature_points_curr.size(); i++) {
      cv::circle(img_curr, feature_points_curr[i], 1, cv::Scalar(0, 0, 255), 2,
                 8, 0);
      cv::line(img_curr, feature_points_cp[i], feature_points_curr[i],
               cv::Scalar(255, 0, 0), 2);
    }
