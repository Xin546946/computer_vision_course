 ...
 */
void GVF::update() {
    cv::Laplacian(gvf_x_, laplacian_gvf_x_, CV_64F, 3, cv::BORDER_REPLICATE);
    cv::Laplacian(gvf_y_, laplacian_gvf_y_, CV_64F, 3, cv::BORDER_REPLICATE);
...
double GVF::compute_energy() {
    ...
    // compute smooth term
    cv::Mat gvf_x_dev_x, gvf_y_dev_x;
    cv::Mat gvf_x_dev_y, gvf_y_dev_y;
    ...
    cv::Mat smooth_term_xx, smooth_term_yx;
    cv::Mat smooth_term_xy, smooth_term_yy;
    cv::pow(gvf_x_dev_x, 2.0f, smooth_term_xx);
    cv::pow(gvf_y_dev_x, 2.0f, smooth_term_yx);
    cv::pow(gvf_x_dev_y, 2.0f, smooth_term_xy);
    cv::pow(gvf_y_dev_y, 2.0f, smooth_term_yy);

    cv::Mat smooth_term =
        smooth_term_xx + smooth_term_yx + smooth_term_xy + smooth_term_yy;

    double smooth_energy =
        param_gvf_.smooth_term_weight_ * cv::sum(smooth_term)[0];
