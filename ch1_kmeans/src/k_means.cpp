...
float check_convergence(std::vector<Center> current_centers, std::vector<Center> last_centers);
...
void Kmeans::run(int max_iteration, float smallest_convergence_rate){
    initial_centers();
    int current_iter = 0;
    while(is_terminate(current_iter, max_iteration, smallest_convergence_rate)){
        current_iter++;
        update_labels();
        update_centers();
...
void Kmeans::initial_centers(){
    std::set<int> random_idx = get_random_index(samples_.size() - 1, centers_.size());
    int i_center = 0;
...
bool Kmeans::is_terminate(int current_iter, int max_iteration, float smallest_convergence_rate) const
{
    float convergence_rate = check_convergence(last_centers_, centers_);
    if (current_iter == max_iteration || convergence_rate < smallest_convergence_rate)
        return true; 
}
...
float check_convergence(std::vector<Center> current_centers, std::vector<Center> last_centers)
{
    ...
    for(int i_center=0; i_center < current_centers.size(); i_center++){
        convergence_rate += calc_square_dist(current_centers[i_center].position_, last_centers[i_center].position_);    
    }
