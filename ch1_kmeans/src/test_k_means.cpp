...
double compute_gaussian_data(const cv::Matx31d& data) {
    cv::Mat tmp;
    cv::exp(0.000005 * (data).t() * (data), tmp);
...
int main(int argc, char** argv) {
    ...
    cv::Mat img = imread(
        "/home/kit/computer_vision_course/ch1_kmeans/images/test_data/"
        "building.jpeg",
        cv::IMREAD_COLOR);
    std::cout << img.at<cv::Vec3b>(3, 3) << '\n';
    std::cout << "********" << '\n';
    std::cout << compute_gaussian_data(img.at<cv::Vec3b>(10, 40)) << '\n';
    return 0;
}
